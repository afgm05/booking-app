...
export default function CoursePage() {
	...
	//For us to be bale to display all the courses from the data file we are going to use the map() method
	const courses = coursesData.map(individualCourse => {
		return(
			<CourseCard key={individualCourse.id} courseProp={individualCourse}/>
			)
		//add key to property to keep track the number of courses and to avoid duplication
	})
